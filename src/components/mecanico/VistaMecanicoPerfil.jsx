...
const VistaMecanicoPerfil = () => {
        ...
        try {
            ...
            if (error) {
                if (error.details?.includes('Result contains no rows')) {
                    setPerfil(null);
                    setPerfilMensaje('No se encontró tu perfil de mecánico. Verifica la tabla mecanicos y el profile_id.');
                } else {
                    throw error;
                }
            } else {
    ...
    const subirFotoPerfil = async () => {
        ...
        try {
            ...
            if (perfil?.foto_mecanico) {
                const oldFileName = perfil.foto_mecanico.split('/').pop().split('?')[0];
                await supabase.storage.from(bucketName).remove([oldFileName]);
            }

            const { error: updateError } = await supabase
                .from('mecanicos')
                .update({ foto_mecanico: nuevaUrl })
                .eq('profile_id', user.id);

            if (updateError) throw updateError;

            setPerfil(prev => ({ ...prev, foto_mecanico: nuevaUrl }));
            setArchivoFoto(null);
            ...
            window.dispatchEvent(new CustomEvent("actualizacion-foto-perfil", { detail: { foto_mecanico: nuevaUrl } }));
    ...
    const eliminarFotoPerfil = async () => {
        if (!perfil?.foto_mecanico) return;
        ...
        try {
            ...
            const { error: updateError } = await supabase
                .from('mecanicos')
                .update({ foto_mecanico: null })
                .eq('profile_id', user.id);

            if (updateError) throw updateError;

            setPerfil(prev => ({ ...prev, foto_mecanico: null }));
            ...
            window.dispatchEvent(new CustomEvent("actualizacion-foto-perfil", { detail: { foto_mecanico: null } }));
    ...
    if (!cargando && perfil === null) return (
        <Container className="text-center py-5">
            <h3 className="text-warning">No se encontró tu perfil de mecánico.</h3>
            <p className="text-muted">{perfilMensaje || 'Verifica que tu cuenta tenga un registro en la tabla mecanicos con el mismo profile_id que tu usuario Supabase.'}</p>
        </Container>
    );
    ...
    return (
        <div className="bg-radial-premium">
            ...
            <Row className="g-4">
                    ...
                    <Card className="perfil-card text-white text-center p-4">
                        <Card.Body>
                            <div className="avatar-container d-flex justify-content-center mb-3">
                                {perfil?.foto_mecanico ? (
                                    <img
                                        src={perfil.foto_mecanico}
                                        alt="Foto de Perfil"
                                ...
                                {!editandoFoto ? (
                                    <>
                                        ...
                                        >
                                            ...
                                            {perfil?.foto_mecanico ? 'Cambiar Foto de Perfil' : 'Agregar Foto de Perfil'}
                                        </Button>
                                        {perfil?.foto_mecanico && (
                                            <Button
                ...
                {/* Columna Derecha: Detalles del Mecánico */}
                <Col lg={8}>
                        ...
                        <Card.Body>
                            ...
                            <div className="perfil-welcome-banner mb-4">
                                ...
                                <p className="mb-0 small text-white-50">
                                    Aquí puedes revisar la información de tu perfil técnico en <strong>Ouroboros Car</strong>. Utiliza el menú principal para ver tus citas asignadas y gestionar el retiro de repuestos.
                                </p>
                            </div>
...
export default VistaMecanicoPerfil;
